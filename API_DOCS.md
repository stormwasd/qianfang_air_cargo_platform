...
#### 21.3 预飞时间与时间及数值显示规范
- **【预飞时间】**：装机预警模板优先展示携程 API 返回的 `ready_time`。若来自数据库原始 4 位时间（如 `"2320"` 或 `"23:20"`），系统统一结合航班日期格式化为 `"YYYY-MM-DD HH:MM"` 格式（如 `"2026-07-27 23:20"`），集装器对应航班时间格式化为 `(HH:MM)`；若无预飞时间则输出 `"/"`。
