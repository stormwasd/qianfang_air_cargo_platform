...
| `permissions` | array[string] | 是 | 用户权限列表（权限代码），用于前端页面权限控制。权限代码包括：`admin`（管理员）、`waybill`（运单管理）、`booking`（订舱管理）、`settlement`（结算单管理）、`customer`（客户管理）、`bill`（单号管理）、`robot`（机器人管理） |
...
- `flight_number` (String, 可选): 航班号，支持模糊或精确查询。
- `page` (Integer, 可选): 页码，默认 1。
- `page_size` (Integer, 可选): 每页数量，默认 10。
...
- 每条记录包含一个嵌套的 `billing_time_containers` 列表，存放关联的计飞时间集装器数据（基于单号后8位关联）。
...
**请求方式**: `POST`
**说明**: 保存或更新深航出港列表的手动录入数据。通过 `waybill_number_8` 关联。

**请求体 JSON**: 
- `waybill_number_8` (String, 必填): 运单号后8位
...
### 1. 获取南航订舱批复跟踪列表
**GET** `/api/v1/departure-tracking/china-southern-air/approvals`

**查询参数：**
- `flight_info` (string, 可选): 订舱航班(模糊匹配)
- `waybill_number` (string, 可选): 运单号(模糊匹配)
- `page` (integer, 默认1): 页码
- `page_size` (integer, 默认10): 每页数量
