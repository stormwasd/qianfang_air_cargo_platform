...
| 单据信息 | POST | `/api/v1/cost-service/consignments/export-excel` | 单据信息-选中下载为 Excel（状态首列、三级分组表头、116 列字段） |
...
6. **Excel 导出规范**：
   - 导出文件使用三行分组表头，数据记录从第 4 行开始，共 116 列；第 1 列为独立的`状态`，表头合并 `A1:A3`，读取费用单据自身的 `status`，`0` 显示`未提交`、`1` 显示`已提交`、`2` 显示`作废`。应收款项包含`运费计算方式`和`燃油费`列，其中`运费计算方式`位于`单价`和`运费`之间，`燃油费`位于`运费`之后；国际空运和国内空运应付款项的`运费计算方式`均位于单价/费率和运费之间。国内空运不导出`航空单位`字段，但接口请求、响应和数据库字段保持不变。
   - 一级分组依次为：`货主托运信息`（第 2-18 列）、`应收款项`（第 19-38 列）、`应付款项`（第 39-110 列）、`折让信息`（第 111-112 列）、`业务信息`（第 113-114 列）、`经营信息`（第 115-116 列）。
   - `应付款项`二级分组顺序与 Web 端一致：`国际空运信息`（第 39-62 列）、`报关信息`（第 63-70 列）、`地面操作信息`（第 71-81 列）、`汽运信息`（第 82-92 列）、`国内空运信息`（第 93-109 列）；第 110 列为独立的`应付合计`。各二级分组的`小计`均位于本分组最后一列、备注之后，对应 `BJ`、`BR`、`CC`、`CN`、`DE` 列，标题与数据同步移动。
   - `国际空运信息`不再导出`托运日期`和`航空公司`列；`国内空运信息`不再导出`航空单位`列；`折让信息`包含第 111 列`折让人员`和第 112 列`折让费`；`报关信息`中不再导出`回扣`列。
