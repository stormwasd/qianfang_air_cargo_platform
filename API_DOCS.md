...
| 单据信息 | POST | `/api/v1/cost-service/consignments/export-excel` | 单据信息-选中下载为 Excel（状态首列、三级分组表头、90 列字段） |
...
6. **Excel 导出规范**：
   - 导出文件使用三行分组表头，数据记录从第 4 行开始，共 90 列；第 1 列为独立的`状态`，表头合并 `A1:A3`，读取费用单据自身的 `status`，`0` 显示`未提交`、`1` 显示`已提交`、`2` 显示`作废`。应收款项包含`运费计算方式`和`燃油费`列，其中`运费计算方式`位于`单价`和`运费`之间，`燃油费`位于`运费`之后。
   - 一级分组依次为：`货主托运信息`（第 2-18 列）、`应收款项`（第 19-38 列）、`应付款项`（第 39-84 列）、`折让信息`（第 85-86 列）、`业务信息`（第 87-88 列）、`经营信息`（第 89-90 列）。
   - `应付款项`二级分组顺序与 Web 端一致：`国际空运信息`（第 39-54 列）、`报关信息`（第 55-60 列）、`地面操作信息`（第 61-69 列）、`汽运信息`（第 70-76 列）、`国内空运信息`（第 77-83 列）；第 84 列为独立的`应付合计`。各二级分组的`小计`均位于本分组最后一列，对应 `BB`、`BH`、`BQ`、`BX`、`CE` 列，标题与数据同步移动。
   - 客户标记为无需导出的 26 个字段为：国际空运的`始发站`、`到达站`、`航班单号`、`航班号`、`航班日期`、`件数`、`运费计算方式`、`备注`；汽运的`托运日期`、`件数`、`体积`、`备注`；国内空运的`托运日期`、`始发站`、`到达站`、`航空公司`、`航空单号`、`航班号`、`航班日期`、`件数`、`运费计算方式`、`备注`；报关的`报关日期`、`其他费用`；地面操作的`托运日期`、`其他费用`。这些字段仅从 Excel 导出移除，请求、响应、数据库和费用登记页面保持不变。
   ...
   - 应收款项的 `freight_method` 在 Excel 导出时按编码转换展示：`1` 显示为`实际重量`，`2` 显示为`计费重量`；国际空运和国内空运的同名字段不再导出，但接口请求、响应及数据库仍保留原始编码值。
   - “提单”列不会直接输出前端保存的编码，而是按页面展示语义转换：
     ...
     - 直单：`2-0 → 直单`；`2-1`不再作为提单类型编码；`2-2`至`2-9`继续分别映射为`直单（虚拟分单*2）`至`直单（虚拟分单*9）`。
     - 兼容当前实际入库格式：新增或修改接口会原样保存前端传入的`bill_of_lading`；除上述数字编码外，导出也识别`一主多分-0`至`一主多分-9`和`直单-0`、`直单-2`至`直单-9`（例如`一主多分-6 → 一主（六）分`、`直单-0 → 直单`、`直单-3 → 直单（虚拟分单*3）`）。`直单-1`不再作为提单类型编码，导出时按未知值原样保留。
   ...
   - 费用登记台导出 `POST /api/v1/cost-service/consignments/export-excel` 的数据行按货主委托信息中的`flight_date`（航班日期）升序排列，即从较早日期到较晚日期；未填写航班日期的记录排在最后，同一航班日期按单据 ID 升序排列。该排序只作用于 Excel 导出，不改变费用登记台列表及客服接单台导出顺序。
   - 应收款项层级不包含“应收-代理”列。响应仍为 `.xlsx` 文件，媒体类型和下载响应头保持不变。

7. **列表日期筛选规范**：
   - `GET /api/v1/cost-service/consignments` 支持以下四个可选日期查询参数，格式均为 `YYYY-MM-DD`：

     | 参数名 | 必填 | 数据字段 | 说明 |
     |---|---|---|---|
     | `start_warehouse_date` | 否 | `consignor_info.warehouse_entry_date` | 进仓日期下限，包含当天 |
     | `end_warehouse_date` | 否 | `consignor_info.warehouse_entry_date` | 进仓日期上限，包含当天 |
     | `start_flight_date` | 否 | `consignor_info.flight_date` | 航班日期下限，包含当天 |
     | `end_flight_date` | 否 | `consignor_info.flight_date` | 航班日期上限，包含当天 |

   - 进仓日期和航班日期是两组相互独立的可选条件。前端可以只传 `start_flight_date`、只传 `end_flight_date`，或同时传航班日期上下限，而无需传任何进仓日期参数。
   - 同一组只传开始日期时查询该日期及之后的数据，只传结束日期时查询该日期及之前的数据；开始和结束日期相同时可精确查询某一天。
   - 两组日期同时传入时采用 AND 关系，记录必须同时满足进仓日期范围和货主托运信息航班日期范围。均不传时不进行日期筛选。
   - 航班日期条件仅匹配货主托运信息的 `flight_date`，不匹配国际空运应付或国内空运应付中的航班日期。
   - 为保证航班日期范围查询性能，`cost_consignments.flight_date` 已增加数据库索引；已有数据库需执行 `sql/migration_add_cost_consignment_flight_date_index.sql`，全新建表脚本已包含该索引。
   - 示例：只查询货主托运航班日期为 2026-09-20 的记录：`GET /api/v1/cost-service/consignments?start_flight_date=2026-09-20&end_flight_date=2026-09-20&page=1&pageSize=10`。
