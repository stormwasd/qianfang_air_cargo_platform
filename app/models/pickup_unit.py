...
class PickupUnit(Base):
    """提货单位管理表"""
    __tablename__ = "pickup_units"
    
    id = Column(BigInteger, primary_key=True, default=generate_id, index=True, comment="主键ID")
    pickup_code = Column(String(50), nullable=True, index=True, comment="提货单位编码")
    pickup_name = Column(String(200), nullable=False, index=True, comment="提货单位名称")
