...
class Waybill(Base):
    ...
    cargo_station_record_status = Column(String(20), nullable=False, default="0", index=True, comment="货站录单执行状态（数据字典值：0=未执行，1=执行中，2=失败，3=已录单）")
    document_print_status = Column(String(20), nullable=False, default="0", index=True, comment="单据打印执行状态（数据字典值：0=未执行，1=执行中，2=失败，3=成功）")
    waybill_void_status = Column(String(20), nullable=False, default="0", index=True, comment="运单作废状态（数据字典值：0=未作废，1=作废中，2=作废失败，3=作废成功）")
    airline_record_time = Column(DateTime(timezone=True), nullable=True, comment="航司录单状态变更时间（中国时间UTC+8）")
    cargo_station_record_time = Column(DateTime(timezone=True), nullable=True, comment="货站录单状态变更时间（中国时间UTC+8）")
    document_print_time = Column(DateTime(timezone=True), nullable=True, comment="单据打印状态变更时间（中国时间UTC+8）")
    ...
    _STATUS_VALUES_WITH_TIME = {"0", "1", "2", "3"}

    @validates("airline_record_status")
    def _on_airline_record_status_set(self, _key, value):
        if value in self._STATUS_VALUES_WITH_TIME:
            self.airline_record_time = get_china_now()
        return value

    @validates("cargo_station_record_status")
    def _on_cargo_station_record_status_set(self, _key, value):
        if value in self._STATUS_VALUES_WITH_TIME:
            self.cargo_station_record_time = get_china_now()
        return value

    @validates("document_print_status")
    def _on_document_print_status_set(self, _key, value):
        if value in self._STATUS_VALUES_WITH_TIME:
            self.document_print_time = get_china_now()
        return value
