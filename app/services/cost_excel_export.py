...
COST_EXPORT_HEADERS: Tuple[str, ...] = (
    ...
    "国空应付-计费重量", "国空应付-单价", "国空应付-运费计算方式", "国空应付-运费", "国空应付-提单费",
    "国空应付-分单费", "国空应付-燃油费", "国空应付-TC费",
    "国空应付-报关费", "国空应付-续页费", "国空应付-耗材费", "国空应付-前置仓",
    "国空应付-其他费用", "国空应付-备注", "国空应付-小计",

    # 应付款项 - 报关（63-70）
    "报关应付-报关日期", "报关应付-报关代理", "报关应付-报关费",
    "报关应付-续页费", "报关应付-查验/删单费", "报关应付-其他费用", "报关应付-备注", "报关应付-小计",

    # 应付款项 - 地面操作（71-81）
    "地面应付-托运日期", "地面应付-外发单位", "地面应付-计费重量",
    "地面应付-费率", "地面应付-运费", "地面应付-提单/快件处置费", "地面应付-安检/报关费",
    "地面应付-打板/退场费", "地面应付-其他费用", "地面应付-备注", "地面应付-小计",

    # 应付款项 - 汽运（82-92）
    "汽运应付-托运日期", "汽运应付-外发单位", "汽运应付-件数",
    "汽运应付-重量", "汽运应付-体积", "汽运应付-单价", "汽运应付-运费",
    "汽运应付-制单费", "汽运应付-其他费用", "汽运应付-备注", "汽运应付-小计",

    # 应付款项 - 国内空运（93-109）
    "国空内应付-托运日期", "国空内应付-外发单位", "国空内应付-始发站",
    "国空内应付-到达站", "国空内应付-航空公司", "国空内应付-航空单号",
    "国空内应付-航班号", "国空内应付-航班日期", "国空内应付-件数", "国空内应付-实际重量",
    "国空内应付-计费重量", "国空内应付-费率", "国空内应付-运费计算方式", "国空内应付-运费", "国空内应付-其他费用",
    "国空内应付-备注", "国空内应付-小计",

    # 应付款项总计、折让信息、业务信息、经营信息（110-116）
...
def append_cost_export_headers(ws: Worksheet) -> List[str]:
    ...
    if len(COST_EXPORT_HEADERS) != 116:
        raise RuntimeError("费用登记导出字段数量异常，预期为 116 列")
    ...
    ws.merge_cells(start_row=1, start_column=39, end_row=1, end_column=110)

    for start_col, end_col in ((39, 62), (63, 70), (71, 81), (82, 92), (93, 109)):
        ws.merge_cells(
            start_row=2,
            start_column=start_col,
            end_row=2,
    ...
    ws.merge_cells(start_row=2, start_column=110, end_row=3, end_column=110)
