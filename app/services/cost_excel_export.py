...
def append_cost_export_headers(ws: Worksheet) -> List[str]:
    ...
    if len(COST_EXPORT_HEADERS) != 117:
        raise RuntimeError("费用登记导出字段数量异常，预期为 117 列")
    ...
    ws.merge_cells(start_row=1, start_column=39, end_row=1, end_column=111)

    for start_col, end_col in ((39, 62), (63, 73), (74, 91), (92, 99), (100, 110)):
        ws.merge_cells(
            start_row=2,
            start_column=start_col,
            end_row=2,
    ...
    ws.merge_cells(start_row=2, start_column=111, end_row=3, end_column=111)
